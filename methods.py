...
def place_cells(rs, sigma=np.pi/2, phases=None, boxsize=2*np.pi):
    """
    rs: (n, 2) array of positions
    phases: (m,) array of place cell phases
    sigma: float, width of the place cell tuning curve
    """
    if type(phases) is int:
        phases = np.random.uniform(-boxsize, boxsize, size=(phases,2))
    activity = np.exp(-np.sum((rs[:, None] - phases[None, :])**2, axis=2)/(2*sigma**2))
    return activity, phases

def border_cells(rs, sigma=np.pi, phases=None, boxsize=2*np.pi):
    """
    rs: (n, 2) array of positions
    phases: (m,) array of border cell phases
    sigma: float, width of the border cell tuning curve
    """
    if type(phases) is int:
        wall_basis = np.array([[0, boxsize], [boxsize, 0], [0, -boxsize], [-boxsize, 0]])
        phases = np.random.choice(4, phases)
        phases = wall_basis[phases]
    dist_to_wall = np.dot(rs, phases.T) - boxsize
    activity = np.exp(-dist_to_wall/(2*sigma**2))
    return activity

def head_direction_cells(vs, sigma=np.pi/10, phases=None):
    """
    vs: (n, 2) array of velocities
    phases: (m,) array of head direction cell phases
    """
    if type(phases) is int:
        phases = np.random.uniform(-np.pi, np.pi, size=phases)
        phases = np.stack([np.cos(phases), np.sin(phases)], axis=1)
    # cosine similarity between velocity phases
    from sklearn.metrics.pairwise import cosine_similarity
    activity = cosine_similarity(vs, phases)
    activity = np.exp(-activity/(2*sigma**2))
    return activity